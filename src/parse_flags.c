...
void	parse_flags_one(char **str, va_list *argp, t_flags *flags)
{
	if (**str == '#')
		flags->sharp = 1;
	if (**str == ' ')
...
void	parse_flags_two(char **str, t_flags *flags)
{
	if (**str == 'B')
		flags->color = 1;
	if (**str == 'Y')
...
void	parse_flags(char **str, va_list *argp, t_flags *flags)
{
	...
	temp = NULL;
	temp = *str;
	while (**str && (**str != 'd' && **str != 'i' && **str != 's' \
		&& **str != 'S' && **str != 'p' && **str != 'D' && **str != 'o' \
		...
		&& **str != 'X' && **str != 'c' && **str != 'C' && **str != '%'))
	{
		...
		if (**str == '+')
			flags->plus = 1;
		parse_flags_one(str, argp, flags);
		parse_flags_two(str, flags);
		*str += (**str) ? 1 : 0;
	}
